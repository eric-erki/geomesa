...
class BulkCopyIndexJob {
  ...
  def run(fsParams: Map[String, String],
    ...
    try {
      ...
      StorageConfiguration.setPath(job.getConfiguration, FileSystemStorageFactory.PathParam.lookup(fsParams))
      StorageConfiguration.setEncoding(job.getConfiguration, FileSystemStorageFactory.EncodingParam.lookup(fsParams))
      val partitions = filter match {
        case None => fsDs.storage.getMetadata(typeName).getPartitions.asScala.toArray
        case Some(f) =>
          fsDs.storage.getPartitions(sft.getTypeName, new Query(typeName, f))
      ...
      val ok = if (!job.isSuccessful) {
        ...
        tempDir.forall(StorageJobUtils.distCopy(_, output, sft, statusCallback, 3, stageCount))
      }
